...
//    ResultPoint.orderBestPatterns(patternInfo);
  ...
  private static final float MIDDLE_SEGMENT_SIZE = 1.0f; // was 3.0f

  /**
   * @param stateCount count of black/white/black/white/black pixels just read
   * @return true iff the proportions of the counts is close enough to the 1/1/1/1/1 ratios
  ...
  protected static boolean foundPatternCross(int[] stateCount) {
    ...
    if (totalModuleSize < SEGMENT_TOTAL_COUNT) {
      return false;
    }
    float moduleSize = totalModuleSize / SEGMENT_TOTAL_COUNT;
   ...
   */
  public static final float SEGMENT_TOTAL_COUNT = 5.0f; // was 7.0f

  protected int max(int... longs) {
  ...
  protected final boolean handlePossibleCenter(int[] stateCount, int i, int j, boolean pureBarcode) {
    int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] +
        stateCount[4];
    int maxCount = max(stateCount);

    float centerJ = centerFromEnd(stateCount, j);
    float centerI = crossCheckVertical(i, (int) centerJ, maxCount, stateCountTotal);
    if (!Float.isNaN(centerI)) {
      // Re-cross check
      centerJ = crossCheckHorizontal((int) centerJ, (int) centerI, maxCount, stateCountTotal);
      if (!Float.isNaN(centerJ) &&
          (!pureBarcode || crossCheckDiagonal((int) centerI, (int) centerJ, maxCount, stateCountTotal))) {
        float estimatedModuleSize = stateCountTotal / SEGMENT_TOTAL_COUNT;
        boolean found = false;
  ...
  private FinderPattern[] selectBestPatterns() throws NotFoundException {
    ...
    return possibleCenters.toArray(new FinderPattern[possibleCenters.size()]);
